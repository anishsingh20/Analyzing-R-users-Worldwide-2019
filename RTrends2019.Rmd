...
# Drilldown(dataset,Filter_by_var,filter_by_val,Select_var,grou_by_var)
DrillDown <- function(data,filt,val,group) {
  
  df <- data %>% filter(filt=="val") %>% 
  group_by(group) %>% 
  summarise(Count=n()) %>%
  arrange(desc(Count))
...
JobSatIndia <- DrillDown(data,Country,"India",JobSat,JobSat)

JobSatUS <-  DrillDown(data,Country,"United States",JobSat,JobSat)

JobSatUk <- DrillDown(data,Country,"Argentina",JobSat,JobSat)


#Career Satisfaction
CarSatIndia <- data %>% filter(data$Country=="India") %>% 
  group_by(data$CareerSat) %>% 
  summarise(Count=n()) %>%
  arrange(desc(Count))

CarSatUS <-  DrillDown(data,Country,"United States",CareerSat)

CarSatUk <- DrillDown(data,Country,"Argentina",JobSat,JobSat)
